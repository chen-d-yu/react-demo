...
const SVGIcon = (props) => {
    ...
    const {width = 16,height=16,iconClass,hoverClass,className,onClick} = props
    ...
    const svgClass = className ? styles.svgIcon + ' ' + className : styles.svgIcon;

    const iconName = hover && hoverClass ? `#icon-svg-${hoverClass}` : `#icon-svg-${iconClass}`;
    const iconStyle = {
        width: `${width}px`,
        height: `${height}px`
    };
